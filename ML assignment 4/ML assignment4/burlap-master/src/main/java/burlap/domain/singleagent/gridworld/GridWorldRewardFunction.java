...
public class GridWorldRewardFunction implements RewardFunction {
	...
	public double reward(State s, Action a, State sprime) {
		...
		int sx = ((GridWorldState)sprime).agent.x;
		int sy = ((GridWorldState)sprime).agent.y;
		...
		if (sx >= this.width || sx < 0 || sy >= this.height || sy < 0) return r;

		if (((sx == width-1 && sy == height-2) || (sx == width-1 && sy == height-1)) && (this.rewardMatrix[sx][sy] != this.rewardMatrix[x][y]))
			r += this.rewardMatrix[sx][sy];
		return r;
	}
